    ...
    },
    {
      ...
      render: (_, record) => (
        <>
          <a
            onClick={() => {
              verificareditacao(record.transaction_id, types).then((result: any) => {
                if (result.message === 'permitido') {
                  setStepFormValues(record);
                  handleUpdateModalVisible(true);
                } else if (result.message === 'reload') {
                  message.success('Usuario já avaliado');
                  updateTable(pages);
                } else if (result.message === 'editando') {
                  message.error('Já existe um processo aberto de validação desse usuário');
                  updateTable(pages);
                } else {
      ...
      {stepFormValues && Object.keys(stepFormValues).length ? (
        <UpdateForm
          ...
          onCancel={() => {
            ...
            cancelareditacao(`${stepFormValues['transaction_id']}`)
              .catch(() => {
