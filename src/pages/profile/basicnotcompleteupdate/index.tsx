...
import { queryUsersDocs, queryUsersDocsIntervalDate, removeRule } from './service';
...
const TableList: React.FC<{}> = () => {
  ...
  const updateTable = (page: number) => {
    queryUsersDocs({ currentPage: page }).then((res: any) => {
      if (res.message) {
        localStorage.clear();
        window.location.href = '/user/login';
      }
  ...
  const teste = async (values: any) => {
    ...
    if (formtkey !== '_timestamp') {
      queryUsersDocsIntervalDate(pages, formtvalue)
        .catch((erro: any) => {
          return erro;
        })
        .then((res: any) => {
